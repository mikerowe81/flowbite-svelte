...
<Wrapper class='relative w-full' show={$$slots.left || $$slots.right}>
    ...
    <slot props={{ ...$$restProps, class: inputClass }}>
        <input {...$$restProps} bind:value bind:this={inputRef} on:blur on:change on:click on:contextmenu on:focus
               on:keydown on:keypress on:keyup on:mouseover on:mouseenter on:mouseleave on:paste on:input {...{type}}
               class={inputClass}/>
    </slot>
    {#if $$slots.right}
